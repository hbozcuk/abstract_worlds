# -*- coding: utf-8 -*-
"""
Streamlit app for abstract inner vs outer worlds with on‐demand SD2.1 loading.
"""
import os
# Turn off Streamlit’s file‐watcher to avoid torch internal errors
...
# ─────────────────────────────────────────────────────────────────────────────
# 1) Manual lazy loader (no decorator)
def load_pipe():
...
# Initialize our pipeline pointer
if "pipe" not in st.session_state:
    st.session_state.pipe = None

# ─────────────────────────────────────────────────────────────────────────────
# 2) Metric function (same as before)
def compute_metrics(path):
    img = Image.open(path).convert("RGB")
    arr = np.array(img)
    hsv = img.convert("HSV"); h,s,v = np.array(hsv).transpose(2,0,1)
...
if st.button("🎨 Oluştur ve Karşılaştır"):
    if not inner_txt or not outer_txt:
        st.warning("⚠️ Lütfen iki metni de girin.")
    else:
        # Lazy‐load only now
        ...
        with st.spinner("🖼️ Görseller üretiliyor…"):
            p1 = f"Soyut sanat eseri… {inner_txt}"
            p2 = f"Soyut sanat eseri… {outer_txt}"
        ...
        # Save & metrics
        img1.save("inner.png"); img2.save("outer.png")
        ...
        ax.plot(angles, np.append(A,A[0]), label="İç", lw=2); ax.fill(angles, np.append(A,A[0]), alpha=0.25)
        ax.plot(angles, np.append(B,B[0]), label="Dış", lw=2); ax.fill(angles, np.append(B,B[0]), alpha=0.25)
        ax.set_xticks(angles[:-1]); ax.set_xticklabels(labels, fontsize=10)
        ax.set_rlabel_position(30); ax.set_title("Son Metrikler", va='bottom', pad=20)
