...
Errors during startup will be caught and displayed in-app.
"""
...
# Abort instantly during Cloud health-check
if os.environ.get("ST_STATE") == "health-check":
    st.stop()

# Wrap everything in try/except to show real errors in the browser
try:
    ##############################################################################
    ...
    if st.button("🎨 Oluştur ve Karşılaştır"):
        if not inner_txt or not outer_txt:
            ...
            labels = [
                ...
                "Renk Canlılığı","Doyg. Sap.","Ton Sap.",
                "Parl. Sap.","Detay"
            ]
